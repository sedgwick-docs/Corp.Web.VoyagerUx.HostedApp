...
- Updated dependency versions (`Corp.Lib.Security.Permissions` 10.1.10, `Corp.Lib.Logging` 10.1.4)
- Made `EnableServerSideNavigationTracking` internal (not configurable from appsettings.json)
- Static assets now copied directly to consuming project's `wwwroot/js/` and `wwwroot/css/` via build targets

### v10.0.1

- Updated `AddVoyagerUxHostedApp` to extend `WebApplicationBuilder` instead of `IServiceCollection`
- `Corp.Web.VoyagerUx.Lib` now bundled in package (no separate dependency required)
- Documentation improvements for Razor Pages navigation history tracking
- Added `<voyagerux-appconfiguration />` tag helper documentation

### v10.0.0

- Initial release for .NET 10
- SharedSessionManager for cookie-based session sharing
- NavigationManager for cross-iframe navigation
- Navigation history tracking via client-side postMessage with `[TrackNavigationHistory]`
- Toast notification system
- Activity tracking for session timeout prevention
- Tag helpers: `<voyagerux-scripts />`, `<voyagerux-tracking-meta />`, `<voyagerux-feature-tests />`, `<voyagerux-appconfiguration />`
- Standalone mode for development
- Pre-built CSS stylesheet matching portal theme
- `EnableServerSideNavigationTracking` option for server-side HTTP tracking
